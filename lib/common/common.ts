...
export const isStatus = (status: string | null): status is Status => status !== null && STATUSES.includes(status);
...
const JOB_EVENT_TYPES: readonly string[] = Object.values(JobEventType);

export const isJobEventType = (event: unknown): event is JobEventType =>
  typeof event === 'string' && JOB_EVENT_TYPES.includes(event);
