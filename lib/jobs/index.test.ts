...
beforeAll(async () => {
  const run = Bun.spawn(
    [
      ...
      // and never collide with any local Valkey instance already running on 6379.
      '-p',
      '127.0.0.1::6379',
...
describe('Jobs', () => {
  ...
  describe('pub/sub', () => {
    it('notifies a channel subscriber when a job is created, updated and deleted', async () => {
      ...
      const unsubscribeDeleted = await JOBS.subscribe(log, namespace, jobId, deleted.listener);
      await JOBS.delete(log, namespace, jobId);
      expect((await deleted.promise).eventType).toBe(JobEventType.DELETED);

      await unsubscribeDeleted?.();
    });
    ...
    it('notifies a pattern subscriber for jobs in a namespace', async () => {
      ...
      const unsubscribe = await JOBS.subscribeNamespace(namespace, event.listener);
      await JOBS.create(log, namespace, jobId);
